...
| Schritt 2 | „Zoom-Wege“ mit links, Mitte und rechts aus „Karten und Dock“: links die Dateien als Karten mit Vorschau und „Alle gleich / Jede einzeln“, Mitte Bahn, Loch und Wege je Datei, rechts die Ziele mit Größe. Die Wahl der Dateiart links ist noch offen (siehe unten). Ausgabe-Einstellungen rechts unter den Zielen (fest: die Variante „Gewählt“ ganz oben in `note-leiste-zonen.html`, D1 kombiniert mit C2): oben die globale Note von 0 bis 100 im Ring mit Wort und Größe; darunter die Größenleiste von Rot bis Grün mit Marken wie „E-Mail“, der Griff ist ein normaler Regler; darunter die Zonen aus C2, anfangs zu: jede zeigt schon zugeklappt ihren Balken (z. B. Schärfe, Details, Bewegung) und öffnet die passenden Regler, dazu „Zielgröße genau“, „Weiteres“ und „Was sich ändert“. Beim Ziehen bewegen sich Note und Balken mit. | `note-leiste-zonen.html` (Gewählt), `einstellungen-final.html` (C2), `artwahl-ausgabe.html` (A3) |
...
- Schritt 2, Wahl der Dateiart links: Richtung steht – je Art ein minimalistisches kleines Universum mit Ring außen herum (wie F2), das schlichte Linien-Symbol in der Mitte, aber in 3D (Röhren mit Tiefe und Glanz, Ring läuft räumlich hinter und vor dem Symbol). Keine Planeten je Datei, nur ein Mond. Drei Oberflächen in `artwahl-universen.html`: I1 Leuchtglas, I2 Farblack, I3 Chrom. Vorschlag: I1. `artwahl-bahnen.html` (G1–G4, zu flach, wirkt aufgesetzt) und `artwahl-3d.html` (H1–H4, andere Richtung) sind Zwischenstände. three.js (MIT) nur im Entwurf; in der App vorgerenderte Bilder oder Windows-Composition, Lizenzprüfung erst bei Verwendung im Code.
