...
- Schritt 2, Wahl der Dateiart links: Keine kreisenden Dateien (bei vielen Dateien überladen), stattdessen ein modernes Menü mit echten 3D-Symbolen je Art. Vier Varianten in `artwahl-3d.html`: H1 Liste, H2 Drehrad, H3 Sockel, H4 Glaskacheln. Vorschlag: H4. Die 3D-Symbole der Entwurfsseite nutzen three.js (MIT) nur im Entwurf; in der App kämen vorgerenderte Bilder oder Windows-Composition zum Einsatz, Lizenzprüfung erst bei Verwendung im Code. Verworfen: `artwahl-bahnen.html` (G1–G4, wirkt aufgesetzt).
