...
### ADR-022 · 2026-09-25 · Aufbau der Zeichenschicht: Szene-Modell ohne WinUI in `Scenes/`, Win2D nur in `Rendering/`, Zeitbasis `TimeSpan`, Weiche über `IMotionSettings`

**Entscheidung:** Jede animierte Fläche nach ADR-018 besteht aus drei Teilen mit fester Grenze. (1) Ein **Szene-Modell** in `Kvertis.App/Scenes/` (erste: `GalaxyScene` für Schritt 1) ohne Win2D- und ohne WinUI-Typen: reine C#-Klassen mit `Update(TimeSpan elapsed)`, Körpern, Teilchen, Kamera und Zeigereinfluss; Farben als eigene `SceneColor`-Struktur, Zufall nur aus einem injizierten `Random`, keine Texte. Die Zeit der Szene ist ausschließlich die Summe der übergebenen `TimeSpan`s (kein `DateTime`, kein `Stopwatch`); jede Glättung hat die Form `1 - exp(-dt * rate)`. Das Modell wird per `Compile Include` in `Kvertis.App.Tests` gebaut und ohne Windows getestet. (2) Ein **Renderer** und eine **Hülle** in `Kvertis.App/Rendering/` (`GalaxyRenderer`, `GalaxyCanvas` um `CanvasAnimatedControl`), die die Szene nur zeichnen; Glühen über radiale Verläufe, keine Blur-Effekte je Bild, keine Bitmaps aus Nutzerdateien (Auflage des Lizenz-Wächters). `Update` und `Draw` laufen auf dem Win2D-Spielschleifen-Thread; der UI-Thread erreicht die Szene nur über eine thread-sichere Kommando-Warteschlange (`Enqueue`) und liest einen unveränderlichen `Snapshot`, der am Ende jedes `Update` ersetzt wird. (3) Ein **Host** in `Views/` (`GalaxyHost`), der über `IMotionSettings` entscheidet, ob die Hülle überhaupt erzeugt wird: bei „Animationen reduzieren“ eine statische XAML-Ansicht mit demselben Informationsgehalt, im Hohen Kontrast gar keine Zeichenfläche (die Artfarben fallen dort zusammen, Ringe trügen keine Information); `IMotionSettings.Changed` schaltet zur Laufzeit um und entfernt das jeweils andere Steuerelement vollständig. Die Szene gehört dem ViewModel und überlebt das Zerstören der Hülle (`Suspend()` = `Paused`, `RemoveFromVisualTree`, Referenz weg; `Resume()` baut neu). Alles Bedienbare und alle Informationen liegen als XAML neben oder über der Fläche; die Fläche selbst ist `AccessibilityView=Raw`.
**Alternativen:** (a) Zeichenlogik direkt in der Hülle (`Update`/`Draw` mit Win2D-Typen), wie im HTML-Entwurf; (b) Szene-Modell im ViewModel mit `DispatcherQueueTimer` als Takt, Zeichnen auf Anforderung (`CanvasControl.Invalidate`); (c) Szene im UI-Thread halten und `RunOnGameLoopThreadAsync` für jeden Zugriff; (d) ein Ordner `Rendering/` für Modell und Zeichner zusammen (so in ADR-018 genannt).
**Grund:** (a) ist nicht ohne Windows testbar und macht Bahnen, Anflug und Zoom von der Bildrate abhängig; die Zahlen aus dem Entwurf (Radien, Umlaufzeiten, Kurven) lassen sich nur an einem reinen Modell gegen feste Zeitpunkte prüfen. (b) taktet auf dem UI-Thread, blockiert bei 60 Bildern je Sekunde die Eingabe und verliert die Gerätewechsel-Behandlung von `CanvasAnimatedControl`. (c) erzeugt je Zeigerbewegung einen Thread-Wechsel und vermischt Lese- und Schreibzugriffe; eine Warteschlange plus Snapshot hat genau zwei Übergabepunkte. (d) ist von ADR-018 überholt: `Scenes/` ohne Win2D lässt sich in eine zweite UI (Avalonia mit anderem Zeichner) übernehmen, `Rendering/` nicht; die Trennung nach Ordner macht die Suche des Reviewers nach verbotenen Win2D-Aufrufen eindeutig.
**Folgen:** Neu `Kvertis.App/Scenes/` (`GalaxyScene`, `GalaxyLayout`, `GalaxyBody`, `GalaxyParticle`, `GalaxyBudget`, `GalaxyMotion`, `GalaxyCommand`, `GalaxySnapshot`, `ScenePalette`, `SceneColor`, `PointerDwell`), neu `Kvertis.App/Rendering/` (`GalaxyRenderer`, `GalaxyCanvas`, `ScenePaletteReader`), neu `Views/GalaxyHost`, `Views/GalaxyStaticView`, `Views/TrayControl`, `Views/TrayFileControl`, `Views/PathsOverlay`; `Views/JobCardControl` entfällt. `Kvertis.App` referenziert `Microsoft.Graphics.Win2D 1.4.0` (Lizenztexte nach `third_party/Win2D/`, Status in `04-bibliotheken.md` „im Code“, Eintrag in `CHANGELOG.md`). Kleine Ergänzungen: `IWorkflowSession.FocusKind` (`MediaKind?`, von `Reset` gelöscht; Nachtrag zu ADR-020) und `FormatRegistry.OutputsFor(FormatId, ISystemCodecCapabilities?)` in der Engine (dieselbe Matrix wie `Suggest`, ohne `InputInfo`). Pixelwirbel, weißes Loch und die Übergangs-Überlagerung folgen demselben Aufbau mit eigenen Szenen (`SwirlScene`, `TransitionScene`) und eigenen Blättern; die Spielereien (Strudel, Urknall) bekommen in dieser Etappe nur den Messpunkt `GalaxyScene.Dwell`. Der Satz „liegt in `Kvertis.App/Rendering`“ in ADR-018 ist damit präzisiert: Modell in `Scenes/`, Zeichner in `Rendering/`. Der Reviewer prüft: kein `Microsoft.Graphics`- und kein `Microsoft.UI`-Namensraum unter `Scenes/`, kein `CanvasBitmap`/`CanvasImageSource` unter `Rendering/`, kein Zugriff auf die Szene außerhalb von `Enqueue`/`Snapshot`/`OrbitAt` vom UI-Thread. Kurzfassung zum Abarbeiten: `docs/entwuerfe/schritt-1-galaxie.md`.

> **Nachtrag zu ADR-022 · 2026-09-25 (Umsetzung):** Die Hülle wird bei der Navigation nicht mehr abgebaut. Grund: `CanvasAnimatedControl.RemoveFromVisualTree()` stürzt mit Win2D 1.4.0 und diesem Windows App SDK reproduzierbar ab (Zugriffsverletzung in `Microsoft.Graphics.Canvas.dll`, auch mit leerem `Draw`), und ohne diesen Aufruf bliebe bei jedem Seitenwechsel ein Steuerelement liegen. Deshalb gilt: Je `GalaxyCanvas` gibt es genau ein `CanvasAnimatedControl` über die Lebensdauer der gecachten Seite (`NavigationCacheMode="Required"`). `Suspend()` und `Unloaded` pausieren nur (`Paused = true`); `Resume()` ist idempotent (die Seite ruft es in `OnNavigatedTo`, der Host in `Loaded`, gebaut wird nur beim ersten Mal oder wenn die Einstellung eine andere Fläche verlangt). Zerstört wird die Fläche nur bei einem Wechsel von `IMotionSettings` oder nach einem Zeichenfehler: dann entsorgt die Hülle den Renderer zuerst per `RunOnGameLoopThreadAsync` auf dem Spielschleifen-Thread (die Pinsel gehören dessen Gerät; Wartezeit höchstens 500 ms), pausiert danach und nimmt das Steuerelement ohne `RemoveFromVisualTree` aus seinem Eltern-Grid. Der Host lässt die alte Fläche so lange im Baum, bis das geschehen ist. Der Pausenzustand (Fenster verborgen, Verlauf offen, anderer Schritt) liegt im Host und wird bei jedem Neuaufbau angewandt. Beim Wechsel Hell/Dunkel werden nur die Pinsel freigegeben (`GalaxyRenderer.ReleaseBrushes`) und auf dem Spielschleifen-Thread sofort neu erzeugt; das Gerät bleibt. `Update` und `Draw` fangen Fehler gleich ab und schalten auf die ruhige Ansicht. Die DPI-Obergrenze (2×) wird bei Größenänderung und bei `XamlRoot.Changed` neu berechnet. Mit einer späteren Win2D-Version, in der `RemoveFromVisualTree` wieder trägt, kann die ursprüngliche Regel (abbauen bei `Suspend`) zurückkehren.

### ADR-023 · 2026-09-25 · Übergangs-Überlagerung als eigene Win2D-Fläche über dem Frame, Geister statt Kopien, Navigation parallel zum Flug, Pixelraster ohne Bild, eine Zeichenschleife zur Zeit

**Entscheidung:** Die Übergänge zwischen den drei Schritten (Wurmloch 1→2, Bogen zurück 2→1, Blätter 2→3) laufen in einer fensterweiten Überlagerung `Rendering/TransitionOverlay` (eigenes `CanvasAnimatedControl`) in `MainWindow` über Schrittleiste und `ContentFrame`, sichtbar nur während eines Übergangs. Die Seiten bleiben unverändert; sie implementieren `ITransitionAnchors` und melden per `TransformToVisual` die Rechtecke ihrer Ziele (Fachköpfe, Arten links, Eingang, Wirbelmitte, Loch) und blenden die beteiligten Elemente während des Flugs aus. Die Überlagerung zeichnet **Geister**: stilisierte Ersatzbilder aus Primitiven (Fachkopf, Artsymbol, Blatt-Attrappe), keine Kopien oder Abbilder der XAML-Elemente. Der Seitenwechsel startet **sofort** (ohne Fluent-Übergang), die Zielseite bleibt unsichtbar, bis der Flug endet; `TransitionService` misst, sperrt Eingaben, steuert die Szene und bricht bei Größenänderung, Fensterverstecken, Themen- oder Bewegungswechsel mit dem Endzustand ab. Für Schritt 3 gilt: Der Pixelwirbel erhält seine Pixel aus einem **rechnerisch erzeugten Raster** (`Scenes/SheetRaster`, 24 × 31 Zellen einer gezeichneten Blatt-Attrappe je Dateiart), nicht aus dem Vorschaubild und nicht aus einer Bitmap; Wirbel, weißes Loch und Abschluss sind eine Fläche (`SwirlScene` mit `WhiteHoleScene` und `FinaleScene`) rechts in `ConvertPage`, das Beben ist eine Composition-Verschiebung von `ContentFrame` und Schrittleiste, der Bericht bleibt die XAML-Karte. Zu jeder Zeit läuft höchstens ein `CanvasAnimatedControl`: der Dienst pausiert Galaxie bzw. Wirbel für die Dauer eines Übergangs. Alle Szenen folgen ADR-022 (WinUI-frei in `Scenes/`, Zeit nur aus `Update(TimeSpan)`, Kommandos und Snapshot). Bei „Animationen reduzieren“ gibt es weder Überlagerung noch Wirbel noch Finale, nur den schlichten Seitenwechsel, die heutige XAML-Ansicht und den Bericht als Text mit einem statischen Ring; im Hohen Kontrast entfällt auch der Ring. Kurzfassung zum Abarbeiten: `docs/entwuerfe/uebergaenge.md`.
**Alternativen:** (a) Die echten XAML-Elemente per Composition-Animation fliegen lassen; (b) Abbilder der Fächer und Symbole per `RenderTargetBitmap` in die Überlagerung laden; (c) Navigation erst nach dem Flug, Zielpositionen aus einem Vorab-Layout schätzen; (d) Übergang als Teil der Galaxie- bzw. Wirbelfläche (kein eigenes Steuerelement); (e) Pixel aus dem Vorschaubild über SkiaSharp abtasten und als Farbfeld übergeben; (f) Galaxie und Überlagerung gleichzeitig laufen lassen.
**Grund:** (a) scheitert daran, dass Quelle und Ziel auf verschiedenen Seiten liegen und mit `Frame` nie gleichzeitig im Baum sind; ein Element kann nicht von einer Seite auf die andere fliegen. (b) würde Vorschaubilder der Nutzerdateien (in den Fächern) als Bitmap in Win2D laden und damit die Auflage des Lizenz-Wächters aus der Win2D-Freigabe verletzen; außerdem wäre das Abbild nicht deterministisch testbar. (c) müsste Zielpositionen raten, die erst nach Layout der Zielseite feststehen; da die Seiten gecacht sind und die Zielseite bis zum Ende des Flugs unsichtbar bleibt, kostet die sofortige Navigation nichts Sichtbares, und die Schrittleiste zeigt den neuen Schritt ohne Verzögerung. (d) bände den Übergang an eine Seite, obwohl er zwei Seiten und den Fensterbereich dazwischen überspannt; ein eigenes, im Leerlauf pausiertes Steuerelement ist einfacher als zwei Flächen, die über ihren Rand hinauszeichnen. (e) bringt eine zweite Bilddekodierung in den UI-Thread und ändert das Ergebnis kaum: der Entwurf tastet selbst nur eine gezeichnete Attrappe ab, und die Pixel nehmen ohnehin zu 95 % die Artfarbe an; ein rechnerisches Raster ist deterministisch, seedbar und unter Linux testbar. (f) verdoppelt Bildrate und Gerätekontexte, und die Zeichenreihenfolge zweier unabhängiger Schleifen ist nicht garantiert; die Galaxie steht während 0,6 bis 2 s ohnehin hinter der Überlagerung.
**Folgen:** Neu `Services/ITransitionAnchors.cs`, `Services/TransitionService.cs` (`ITransitionService`, Singleton, eingehängt in `StepNavigationService.GoTo`; `FrameNavigationService.Navigate` bekommt einen optionalen Parameter für die Übergangsart), `Rendering/TransitionOverlay`, `Rendering/TransitionRenderer`, `Rendering/SwirlCanvas`, `Rendering/SwirlRenderer`, `Rendering/FinaleRenderer`, `Views/SwirlHost`, `Views/FinaleStaticView`; neu in `Scenes/`: `SceneMotion` (CSS-`cubic-bezier`-Löser, Keyframe-Auswertung), `TransitionScene`, `SheetRaster`, `SwirlScene`, `WhiteHoleScene`, `FinaleScene`, `SwirlBudget`; `ScenePalette` erweitert um `LineStrong`, `Paper`, `PaperLine`, `OnMint`, `Shadow`; `MainPage`, `TargetPage`, `ConvertPage` implementieren `ITransitionAnchors`; `MainWindow` hält die Überlagerung; `ConvertPageViewModel` besitzt die `SwirlScene` und übersetzt `IConversionCoordinator.Changed` in Szene-Kommandos; `CardAnimations` bekommt `PageShake`, `FadeIn`, `FadeOut`. Keine neue Bibliothek, Engine und Queue unverändert. Der Reviewer prüft: kein `RenderTargetBitmap`, `CanvasBitmap`, `CanvasImageSource` unter `Rendering/`; kein `Microsoft.Graphics`/`Microsoft.UI` unter `Scenes/`; `TransitionService` pausiert die Fläche der Quellseite vor `Begin`; `IsTransitioning` wird in allen Navigationsbefehlen geprüft. Die Zahlen (Dauern, Stationen, Kurven, Konstanten `PH1`, `PH2`, `M`, `STILL`, `MAXW`) stehen im Arbeitsblatt und dürfen nach Sichtprüfung angepasst werden; die Form (Stationen mit Easing je Abschnitt, Tabelle des Tanzes, Deckel 16° je Bild) nicht ohne Nachtrag hier.

> **Nachtrag zu ADR-023 · 2026-09-25 (Reviewer):** Das Beben nach der Supernova bewegt nicht nur `ContentFrame` und Schrittleiste, sondern wie im Mischentwurf auch die Zeilen der Dateiliste einzeln und versetzt; gezeichnet werden höchstens die ersten 12 Zeilen, weitere beben nicht. Die Streichung im Arbeitsblatt (`docs/entwuerfe/uebergaenge.md`, Zeile „Beben“) ist damit zurückgenommen; die Abweichungsliste steht in `docs/06-design.md`.
