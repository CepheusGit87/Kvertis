...
**Nachtrag Teil B · Spielereien Strudel und Urknall (2026-09-25):** Die Zeile „Spielereien in Schritt 1“ aus `design/ENTSCHEIDUNGEN.md` ist umgesetzt, Vorlage `design/faecher-galaxie.html` (`planetTick`, `wirbelPos`, `planetZeichnen`, `urknallTick`, `urknallPos`, `urknallLoch`, `uiSog`, `planetenSog`) mit denselben Zahlen. Szene-Modell WinUI-frei als Teilklasse `Scenes/GalaxyScene.Gimmicks.cs` mit `Scenes/GalaxyPlanet.cs` (`GimmickState` Ruhe/Strudel/Blitz/Neuer Planet/Sog/Einschläge/Einsaugen/Urknall/Neues Universum, `PlanetKind` sechs Arten im Wechsel, `GalaxyPlanet`, `GalaxyBurst`); Zeitachse nur aus `Update(TimeSpan)`: Strudel 4 s ab 0,15 s ruhigem Zeiger (`PointerDwell`), Blitz 0,8 s, Planet federt 0,7 s ein; Loch 10 s, Einsaugen ab 80 % der Ladung (letzte 2 s), Urknall 1,4 s, neues Universum 3,4 s; Abbau bei Abbruch 1,1/s (Strudel weg vom Zeiger), 1,4/s (Bewegung), 0,6/s (Loch). Teilchen aus dem bestehenden Budget: 44 je Planet (`GalaxyBudget.HaloPerPlanet`), höchstens 12 Planeten (`MaxPlanets`), harte Grenze 4000 bleibt (darüber Planet ohne Hof). Die Oberfläche wird nie gezeichnet: die Szene liefert im `GalaxySnapshot` `SurfaceSuction` (0…1), `SurfaceOpacity`, `SurfaceJitter`, `HoleCenter`; `GalaxyCanvas` meldet `SurfaceEffectChanged`, und `GalaxyHost` zieht die fünf `TrayControl` (über den Visualbaum der Seite gefunden), Summe, Ablage-Karte, „Nicht umwandelbar“ und Hinweiszeile als `UIElement.Translation`/`Scale`/`Rotation`/`Opacity` zum Loch (Verschiebung × Sog, Drehung bis 50°, Skalierung bis 4 %, Zittern), in `CompositionTarget.Rendering` nur solange der Sog wirkt, und setzt alles beim Ende exakt zurück. Nach dem Urknall bleiben alle Körper unverändert in der Szene (nur Höfe fliegen hinaus und kehren in der Spirale zurück; Test prüft Positionen bis 0,05 px); selbst geformte Planeten sind weg. Nur in der Übersicht (`Zoom == Overview`, keine Kamerafahrt, kein Ziehen, keine anfliegenden Dateien); der Host schaltet die Spielereien nur für die bewegte Fläche frei (`SetGimmicksEnabled`), also nie bei „Animationen reduzieren“ oder Hohem Kontrast; beim Pausieren (anderer Schritt, Verlauf, Fenster verborgen) geht `SetPointer(null)` an die Szene, damit keine Ladung weiterläuft. Zeichner: Strudel als drei gestrichelte Ellipsen mit Trichter (Win2D-Striche in Strichbreiten umgerechnet), Planeten als beschnittene Kugel (Ebene `CreateLayer`) mit Bändern, Kratern, Wolken, Schattierung als radialer Verlauf mit Lichtpunkt zum Loch, Ring und Monde vorn und hinten; Einschlag-Ringe und Urknall-Funken als Glühen und Ellipsen; Sterne des neuen Universums sind die zurückkehrenden Teilchen (Alpha 0,12 → 1). Tests `Kvertis.App.Tests/Scenes/GalaxyGimmickTests.cs` (12). Bewusste Abweichungen: **Bewegung > 6 px setzt den Strudel ganz zurück** (weich auf 0 und neu von vorn; der Entwurf lässt ihn nur während der Bewegung abklingen) – so verlangt vom Auftrag „setzt zurück“, und ein kurzer Ruck kostet dann keinen halben Strudel, sondern die ganze Wartezeit. **Am Loch zählt die Lage, nicht die Stille:** die Ladung wächst, solange der Zeiger auf dem Loch ruht (`PointerDwell.OnHole`, Radius + 12 px), kleine Bewegungen darin setzen nicht zurück (wie im Entwurf, `nah < 36`), Verlassen baut weich ab. **Kein Schriftzug „ein neues Universum“** unter dem Loch: Text in der Zeichenschicht müsste aus den Ressourcen kommen und der Zeichner erhält keine Texte außer den Schildern; die Wiedergeburt ist ohne Beschriftung verständlich. **Eingesogen werden nur die Elemente von Schritt 1** (Fächer und die Karten des Hosts), nicht Schrittleiste, Fußleiste und Fensterleiste des Entwurfs (`UI_SEL`): `MainWindow` und `MainPage` gehören parallel anderen Aufgaben, und ein Host, der die Fensterleiste verschiebt, wäre ein Schichtbruch. **Kein Zeiger-Ausblenden** (`cursor: none` beim Laden) und kein CSS-Beben der Seite beim Einschlag: das Zittern der Zeichenfläche (Loch und Teilchen) übernimmt den Stoß.
...
**Nachtrag Teil D · Staubringe ab 150 Dateien (2026-09-25):** Der offene Punkt „Ab etwa 150 Dateien Übergang in Staubringe“ (`design/ENTSCHEIDUNGEN.md` „Ziel in Schritt 3“, `docs/entwuerfe/uebergaenge.md` „Staubringe ab ~150 Dateien“) ist nach Entwurf 2 „Staubringe“ aus `design/weisses-loch-mengen.html` umgesetzt. `Scenes/WhiteHoleScene` hat den Modus `WhiteHoleMode` (`Rings` unter 150, `Dust` ab `DustThreshold = 150` Dateien ohne eigenes Ziel), deterministisch aus Plan und Zeit (Körner und Plätze über einen Hash, kein Zufall). Staubmodus: ein Ring je Dateiart in Anzeigereihenfolge (`TargetPlanner.KindOrder`), `rx = R0 + j·dr` mit `R0 = 62` (eine Art) sonst 36, `dr = min(20, (tw/2 − 70)/max(1, Arten − 1))`, Streuung ±13 (eine Art) sonst ±4,5, Drehung `t·0,35·(36/rx)^1,2`; gestrichelte Ringlinie 2/5 `KvLineStrong` Alpha 0,55; Glühband je Ring in Artfarbe, Alpha `0,1 + 0,12·Füllung`, Breite `18·Füllung + 4` (eine Art) sonst `6·Füllung + 2`, dunkel additiv; Körner je Datei `G = clamp(round(700/N), 10, 64)` als 1,7-px-Quadrate Alpha 0,85, die mit jeder Ankunft anteilig aufleuchten; Ankunftspuls als Ellipse `5 + k·rx` (0,8 s, Alpha `1 − k`); Pixelstrom landet auf einem festen Punkt im Ring seiner Art. Zähler „n von N“ unter `R = min(tw/2 − 8, 132)`. Abschluss (`FinaleScene.IsDust`): die Ringe ziehen sich mit `skalVon` zusammen und verblassen mit den Bahnlinien, der Sog nimmt die Ringradien; statt eines Planeten je Datei kreist je Art ein Sammelplanet (Radius 4,5, blendet in 0,35 s ein, `FinalePlanet.Count`), bei Fehlern je Art zusätzlich ein Korallen-Ring gegenüber; im Ring am Ende steht die Zahl unter jedem Sammelplaneten (`SwirlRenderer.DrawPlanetCount`, Mono 11 `KvInk`, nur Ziffern, kein Text). Budget: neuer Wert `SwirlBudget.RingGrains = 2400` als gemeinsamer Vorrat der Ringe (`min(G·Anzahl, Vorrat·Anzahl/N)` je Art), zusätzlich nimmt die Szene je Bild nur, was Pixel, Sog und Funken vom harten Deckel 4000 übrig lassen (`ParticleCount` zählt die Körner mit; im großen Lauf gemessen 4000, nie darüber); nach dem Verblassen der Ringe fallen die Körner weg. Bewusste Abweichungen vom Entwurf: **kein Fortschrittsbogen** aus Stücken je Datei um die Ringe (bei 500 Dateien 500 Bogenstücke je Bild; Glühband und Zähler tragen die Information), **fehlgeschlagene Dateien bleiben als Korallen-Ringe auf ihrem Artring** sichtbar (Entwurf: nicht unterscheidbar; Deckel `MaxEmptyPlaces`), **Sammelplaneten mit Zahl** statt eines Planeten je Datei im Abschlussring (500 Planeten wären unlesbar und die Trägheitssimulation zu teuer; 60 s Finale mit 500 Dateien rechnen in unter 2 s). Ruhige Ansicht (`FinaleStaticView`) unverändert. Debug-Hilfen (nur `#if DEBUG`): `KVERTIS_STAGE_FILES=@liste.txt` liest die Pfade aus einer Datei; `KVERTIS_GALAXY_STATS=1` schreibt alle 5 s eine Zeile mit ms/Bild (Mittel, letzte 60 Bilder, Maximum) und Teilchenzahl nach `logs/stats.log`. Tests `Kvertis.App.Tests/Scenes/DustRingsTests.cs` (Moduswechsel 149/150/500, Körner je Datei, Ringe, Ankunft, Fehler, Pixelziel, Budget, Finale, Laufzeit).
