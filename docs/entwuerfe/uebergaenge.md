...
| „n von N“ | bleibt XAML (`DoneText`, `LiveSetting=Polite`); die Szene zeichnet keine Zahl | eigene Festlegung |
...
| Beben | `e = 0` | **Canvas:** `e < 0,45` Versatz der ganzen Zeichnung `(sin(97e)·k, cos(83e)·0,6k)`, `k = 5·(1 − e/0,45)`. **Seite (XAML):** Composition-`Offset`-Animation auf `ContentFrame`: Stationen (−6, 4), (5, −3), (−3, 2), (1,5, −1), 0 über 520 ms `ease-out`; Kopfleiste (`StepHeader`) Stationen (5, 3) mit 0,4° Drehung, (−5, 0), (3, 0), (−1,5, 0), 0 über 700 ms ab 40 ms, `cubicBezier(.3, .7, .4, 1)`. Die Zeilen der Liste beben **nicht** einzeln (Entwurf: je Zeile versetzt; hier gestrichen, weil die Seite als Ganzes bebt und `ItemsRepeater`-Zeilen keine eigene Animation brauchen). Nur einmal je Runde, nie bei Reduced Motion | `beben` |
