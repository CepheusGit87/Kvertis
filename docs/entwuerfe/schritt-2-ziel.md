...
Namensraum `Kvertis.Engine.Tuning`: `QualityGrade`, `GradeBand`, `TuningAspect`, `AspectLevel`, `AspectDetail`, `GradeMapper`, `EffectCode`, `EffectSeverity`, `ConversionEffect`, `EffectAnalyzer`, `GradeSizeTable`, `GradeSizeTableBuilder`, `SizeMarks`. Dazu `Estimation.SizeModel` und die Anpassung von `Estimator`. Signaturen und Abbildungstabelle stehen in `03-architektur.md`; die Umsetzung weicht in vier Punkten ab (Nachtrag im Skizzen-Abschnitt): Stufenleitern feiner (Schärfe 7, Bildrate 6 Stufen), Note nur bis auf Toleranz rückrechenbar, `SizeModel`-Referenz ist der Auslieferungszustand von `ConversionSettings`, `SizeMarks.For` liefert `SizeMark`-Records, verlustfreie Bilder behalten die Schärfe-Zone, `SupportsGrade` ist bei FLAC/WAV/AIFF falsch. Tests: `GradeOf` monoton und bis auf eine Toleranz (20 Punkte bei AAC, sonst 12) an der Note, Tabelle monoton, kein Wurf für alle Kombinationen der `FormatRegistry`. Ohne diese Etappe bindet die Ziel-Seite an nichts.
...
- Regeln:
  ...
  - Leiste: logarithmische Skala von `MinBytes` (kleinste Tabelle) bis zur größten Quelldatei; Position → Bytes → `GradeForBytes` der größten Datei. Marken aus `SizeMarks.For(kind)` (`SizeMark.Preset`, `SizeMark.Bytes`), Beschriftung `Preset_Messenger`, `Preset_Email` aus den Ressourcen; Marken außerhalb der Skala nicht zeichnen.
  - „damals“-Note und Note nach einer Zonen-Änderung kommen aus `GradeOf` und sind nur bis auf Toleranz genau: im Ring mit „≈“ anzeigen. Bei verlustfreiem Audio (`SupportsGrade = false`) entfallen Ring, Leiste und Zonen; bei PNG/TIFF gibt es die Zone „Schärfe“, „Details“ ist fest.
