...
| `ModelConverter` | STL, 3MF, OBJ, PLY, GLB lesen und schreiben; glTF (.gltf) lesen | eigener Code (.NET: `System.IO.Compression`, `System.Xml`, `System.Text.Json`) | MIT (.NET) | keine: offene, lizenzfreie Formate (STL, OBJ, PLY frei; 3MF-Kernspezifikation des 3MF-Konsortiums; glTF 2.0 von Khronos, lizenzfrei) | Keine Bibliothek, keine native DLL; Erweiterungen mit eigener Lizenzlage (z. B. Netzkompression) werden abgelehnt statt eingebaut; keine Herstellerformate (FBX, USDZ); keine verknüpften Dateien, kein Netzwerk (glTF-Puffer nur aus dem Modellordner) | ADR-016; `ModelConverterTests` (Rundreise aller Formate, Einheiten und Achsen, Ablehnung fremder Pfade und Erweiterungen, Grenzwerte) |
